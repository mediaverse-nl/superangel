...
class LS extends \Singleton {
  ...
  public static $default_config = array(
     ...
     */
      "info" => array(
          "company" => "Superangel.nl",
          "emailContact" => "contact@superangel.nl",
          "emailNoreply" => "noreply@superangel.nl"
      ),
  ...
  public static function init() {
    ...
    if(self::$loggedIn === true && array_search(self::curPage(), self::$config['pages']['no_login']) !== false){
      ...
      self::redirect(\Error\ES::GetResponsCode(403));
    }
  ...
  public static function register( $id, $password, $other = array() ){
    ...
    if( self::userExists($id) ){
    ...
    }elseif( isset($other['email'])&& self::userExists($other['email']) ){
      return "existsEmail";
    }else{
      ...
      if( count($other) == 0 ){
        ...
        $sql = self::run("INSERT INTO `". self::$config['db']['table'] ."` (`username`, `password`, `password_salt`) VALUES(:username, :password, :passwordSalt)");
      }else{
        ...
        $sql   = self::run("INSERT INTO `". self::$config['db']['table'] ."` (`username`, `password`, `password_salt`, $columns) VALUES(:username, :password, :passwordSalt, :$colVals)");
  ...
  public static function faq(){
    ...
    $sql = self::run("SELECT * FROM faq");

    $sql->execute();

    while($row = $sql->fetch(\PDO::FETCH_ASSOC)){
      $html_faq .= '<div class="panel panel-default">
                      <div class="panel-heading">
                        <h4 class="panel-title">
                          <a data-toggle="collapse" data-parent="#accordion" href="#collapse'.$row['f_id'].'">'.$row['f_title'].'</a>
                        </h4>
                      </div>
                      <div id="collapse'.$row['f_id'].'" class="panel-collapse collapse">
                        <div class="panel-body">'.$row['f_desc'].'</div>
                      </div>
                    </div>';
    }
    return $html_faq;
  ...
  public static function forgotPassword(){
    ...
    if( !isset($_POST['logSysForgotPass']) && !isset($_GET['resetPassToken']) && !isset($_POST['logSysForgotPassChange']) ){
      ...
      $sql = self::run("SELECT `uid` FROM `resetTokens` WHERE `token` = ?");
      ...
      if( $sql->rowCount() == 0 || $reset_pass_token == "" ){
        ...
        if($_POST['logSysForgotPassNewPassword'] == "" || $_POST['logSysForgotPassRetypedPassword'] == ""){
          ...
          if(self::changePassword("", $_POST['logSysForgotPassNewPassword'])){
             ...
             */
            $sql = self::run("DELETE FROM `resetTokens` WHERE `token` = ?");
            $sql->execute(array($reset_pass_token));
      ...
      if($identification == ""){
        ...
        $sql = self::run("SELECT `email`, `id` FROM `". self::$config['db']['table'] ."` WHERE `username`=:login OR `email`=:login");
        ...
        if($sql->rowCount() == 0){
          ...
          $sql = self::run("INSERT INTO `resetTokens` (`token`, `uid`, `requested`) VALUES (?, ?, NOW())");
  ...
  public static function changePassword($curpass, $newpass, $parent = ""){
    ...
    if(self::$loggedIn){
      ...
      $sql = self::run("UPDATE `". self::$config['db']['table'] ."` SET `password` = ?, `password_salt` = ? WHERE `id` = ?");
  ...
  public static function personalInfoChecker(){
    self::construct();
  ...
  public static function getUser($what = "*", $user = null){
    ...
    $sql = self::run("SELECT {$columns} FROM `". self::$config['db']['table'] ."` WHERE `id` = ? ORDER BY `id` LIMIT 1");
  ...
  public static function updateUser($toUpdate = array(), $user = null){
    ...
    if( is_array($toUpdate) && !isset($toUpdate['id']) ){
      ...
      $sql = self::run("UPDATE `". self::$config['db']['table'] ."` SET {$columns} WHERE `id`=:id");
  ...
  public static function sendMail($email, $subject, $body){
    ...
    $headers[] = "From: ". self::$config['info']['emailNoreply'];
    $headers[] = "Reply-To: ". self::$config['info']['emailNoreply'];
  ...
  public static function GetIp(){
    ...
    if(self::$loggedIn = true){
      ...
      if (!empty($_SERVER['HTTP_CLIENT_IP'])){
        ...
        $true_ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
      }else{
